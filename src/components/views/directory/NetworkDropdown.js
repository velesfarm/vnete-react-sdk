...
const NetworkDropdown = ({
    ...
    const [userDefinedServers, _setUserDefinedServers] =
        useState(_userDefinedServers);
    ...
    if (menuDisplayed) {
        ...
        const removableServers = new Set(
            userDefinedServers.filter(
                (s) => !configServers.has(s) && s !== hsName
            )
        );
        const servers = [
            // we always show our connected HS, this takes precedence over it being configured or user-defined
            hsName,
            ...Array.from(configServers)
                .filter((s) => s !== hsName)
                .sort(),
        ...
        const options = servers.map((server) => {
            ...
            const protocolsList =
                server === hsName ? Object.values(protocols) : [];
            ...
            if (removableServers.has(server)) {
                const onClick = async () => {
                    ...
                    const { finished } = Modal.createTrackedDialog(
                        ...
                        QuestionDialog,
                        {
                            ...
                            description: _t(
                                "Are you sure you want to remove <b>%(serverName)s</b>",
                                {
                                    serverName: server,
                                },
                                {
                ...
                removeButton = (
                    <MenuItem onClick={onClick} label={_t("Remove server")} />
                );
            }
        ...
        content = (
            <ContextMenu {...inPlaceOf(buttonRect)} onFinished={closeMenu}>
    ...
    return (
        <div className="mx_NetworkDropdown" ref={handle}>
            {content}
        </div>
    );
};
